document.addEventListener('DOMContentLoaded', () => {
    ...
    function moveLeft() {
        undraw()
        //left edge => 0,10,20,30...
        //0%10=0
        const isAtLeftEdge = current.some(index => (currentPosition + index) % width === 0)
        if (!isAtLeftEdge) currentPosition -= 1
        //preventing the tetrominoes to move over other tetrominoes
    ...
    //rotate the tetromino by pushing up button
    function rotate() {
        undraw()
        currentRotation = ++currentRotation == current.length ? 0 : currentRotation
        current = theTetrominoes[random][currentRotation]
        draw()
    }
